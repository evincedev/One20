...
class Index extends \Magento\Framework\View\Element\Template
{
    ...
    public function getTirewidthOptions($code){
    	if(isset($_SESSION['tirewidth'])){
    	$data = unserialize($_SESSION['tirewidth']);
    	}
        if($data == ""){
		$collection = $this->_productCollectionFactory->create()->setVisibility($this->_visibleProduts->getVisibleInCatalogIds())
		->addAttributeToSelect($code)->addAttributeToFilter('status', array('in' => array(1) ))->joinField('stock_item', 'cataloginventory_stock_item', 'qty', 'product_id=entity_id', 'qty!=0')->joinField('position', 'catalog_category_product_index', 'position','product_id=entity_id', 'cat_index.position!=0');
        $optionids = array();
        foreach ($collection as $value) {
        	if($value->getData($code))
        	$optionids[] = $value->getData($code);
        }
        sort($optionids);
		$manufacturerOptions = $this->_productAttributeRepository->get($code)->getOptions();       
        $values = array();
        foreach ($manufacturerOptions as $manufacturerOption) { 
        	if (in_array($manufacturerOption->getValue(), array_unique($optionids))){
            $values[$manufacturerOption->getValue()] = $manufacturerOption->getLabel();  // Label
            }
        }
		$_SESSION['tirewidth'] = serialize($values);
        return $values;
    } else {
    	return $data;
     }
    ...
    public function getAspectRatioOptions($code){
        if(isset($_SESSION['aspectratio'])){
        $data = unserialize($_SESSION['aspectratio']);
        }
        if($data == ""){
        $collection = $this->_productCollectionFactory->create()->setVisibility($this->_visibleProduts->getVisibleInCatalogIds())
        ->addAttributeToSelect($code)->addAttributeToFilter('status', array('in' => array(1) ))->joinField('stock_item', 'cataloginventory_stock_item', 'qty', 'product_id=entity_id', 'qty!=0')->joinField('position', 'catalog_category_product_index', 'position','product_id=entity_id', 'cat_index.position!=0');
        ...
        $_SESSION['aspectratio'] = serialize($values);
        return $values;
    } else {
        return $data;
     }
    ...
    public function getRimdiameterOptions($code){
        if(isset($_SESSION['rimdia'])){
        $data = unserialize($_SESSION['rimdia']);
    	}
        if($data == ""){
		$collection = $this->_productCollectionFactory->create()->setVisibility($this->_visibleProduts->getVisibleInCatalogIds())
		->addAttributeToSelect($code)->addAttributeToFilter('status', array('in' => array(1) ))->joinField('stock_item', 'cataloginventory_stock_item', 'qty', 'product_id=entity_id', 'qty!=0')->joinField('position', 'catalog_category_product_index', 'position','product_id=entity_id', 'cat_index.position!=0');
        ...
        $_SESSION['rimdia'] = serialize($values);
        return $values;
    	} else {
    	return $data;
     }
